...
function toggleCycleSettings() {
	...
	settings.style.display = document.getElementById('cycle-mode').checked
		? 'block'
		: 'none';
}
...
async function startChatMailing() {
	...
	try {
		...
		const CycleEnabled = document.getElementById('cycle-mode').checked;
		formData.append('cycle', document.getElementById('cycle-mode').checked);
		if (CycleEnabled) {
			
			formData.append(
				'delay_cycle',
				document.getElementById('delay_cycle').value
			);
		}
		...
		if (
			document.getElementById('attach-file').checked &&
		...
		// Отправляем запрос
		const response = await fetch('/startchatmailing', {
			method: 'POST',
			body: formData,
		});
