...
document.addEventListener('DOMContentLoaded', () => {
    ...
    initMasslookSettings();

			// Обработчики изменений
    document
        .querySelectorAll('input[name="masslook-type"]')
        .forEach(radio => {
            radio.addEventListener('change', handleMasslookTypeChange);
        });
...
async function startMasslook() {
	...
	try {
		...
		// Сбор данных
		const settings = {
			threads: parseInt(document.getElementById('threads-count').value),
			reactions: document
				.getElementById('story-reactions')
				.value.split(',')
				.map(r => r.trim()),
			type: document.querySelector('input[name="masslook-type"]:checked').value,
			chats: [],
			usersSettings: {},
		};

		if (settings.type === 'chats') {
			settings.chats = document
				.getElementById('chats-list')
				.value.split('\n')
				.filter(c => c.trim());
			if (settings.chats.length === 0) throw new Error('Добавьте чаты');
		} else {
			settings.usersSettings = {
				viewDelay: document.getElementById('view-delay').value,
				reactionDelay: document.getElementById('reaction-delay').value,
				usersPerAccount: parseInt(
					document.getElementById('users-per-account').value
				),
				storiesLimit: parseInt(document.getElementById('stories-limit').value),
				usersList: document
			...
			if (!validateDelay(settings.usersSettings.viewDelay))
				throw new Error('Некорректная задержка просмотра');
			if (!validateDelay(settings.usersSettings.reactionDelay))
				throw new Error('Некорректная задержка реакций');
			if (settings.usersSettings.usersList.length === 0)
				throw new Error('Добавьте пользователей');
		}
		...
		// Отправка запроса
		const response = await fetch('/start_masslook', {
			method: 'POST',
			body: formData,
		});
		closeModal();
		...
		const result = await response.json();
		showNotification(result.message || 'MassLook успешно запущен!', 'success');
		closeModal();
		updateMasslookStatus(true);
	} catch (error) {
...
function updateMasslookStatus(isActive) {
	const statusElement = document.getElementById('masslook-status');
	if (statusElement) {
...
document.addEventListener('DOMContentLoaded', function () {
	...
	// Обработчики изменений
	document.querySelectorAll('input[name="masslook-type"]').forEach(radio => {
		radio.addEventListener('change', handleMasslookTypeChange);
	});
});

function initMasslookSettings() {
	// Показываем правильные настройки при загрузке
	const initialType = document.querySelector(
		'input[name="masslook-type"]:checked'
	).value;
	toggleSettingsVisibility(initialType);
}

function handleMasslookTypeChange(event) {
	toggleSettingsVisibility(event.target.value);
}

function toggleSettingsVisibility(type) {
	const chatsSettings = document.getElementById('chats-settings');
	const usersSettings = document.getElementById('users-settings');

	if (type === 'chats') {
		chatsSettings.style.display = 'block';
		usersSettings.style.display = 'none';
	} else {
		chatsSettings.style.display = 'none';
		usersSettings.style.display = 'block';
	}
}